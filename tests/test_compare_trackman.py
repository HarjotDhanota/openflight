...
class TestHeaderAliases:
    def test_standard_headers_resolve(self):
        ...
        assert ct._build_column_map(["Club Path", "Attack Angle"]) == {
            "club_path_deg": "Club Path",
            "attack_angle_deg": "Attack Angle",
        }

    def test_delivery_fields_load_from_trackman_export(self, tmp_path):
        path = tmp_path / "delivery.csv"
        _write_trackman_csv(
            path,
            ["Shot Number", "Club", "Club Path", "Attack Angle"],
            [{"Shot Number": "1", "Club": "7-iron", "Club Path": "-2.4", "Attack Angle": "-4.1"}],
        )
        shot = ct.load_trackman(path)[0]
        assert shot.club_path_deg == pytest.approx(-2.4)
        assert shot.attack_angle_deg == pytest.approx(-4.1)
