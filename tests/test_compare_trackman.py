...
#
# Ball metrics were comparable from the start; club metrics were not, which
# meant no face-angle or impact-location figure could ever be checked against
# truth. These tests cover the club side of that comparison.
# ---------------------------------------------------------------------------


class TestClubMetrics:
    def test_every_club_metric_has_a_shot_field(self):
        """The metric table drives Shot, the aliases and the CSV alike, so a
        metric declared without a field would silently never be compared."""
        for metric in ct.CLUB_METRICS:
            assert hasattr(
                ct.Shot(source="of", shot_number=None, timestamp=None, club=""), metric.field
            ), metric.field

    def test_trackman_club_columns_are_recognised(self, tmp_path):
        path = tmp_path / "tm.csv"
        _write_trackman_csv(
            path,
            [
        ...
        assert len(shots) == 1
        got = shots[0]
        assert got.attack_angle == pytest.approx(-4.1)
        assert got.club_path == pytest.approx(2.3)
        assert got.face_angle == pytest.approx(-1.2)
        assert got.face_to_path == pytest.approx(-3.5)
        assert got.dynamic_loft == pytest.approx(27.4)
        assert got.impact_height_mm == pytest.approx(3.0)
        assert got.impact_offset_mm == pytest.approx(-2.5)
        assert got.spin_axis == pytest.approx(-5.0)

    def test_openflight_club_fields_are_loaded(self, tmp_path):
        path = tmp_path / "of.jsonl"
        _write_openflight_jsonl(
            path,
            [
                {
                    "timestamp": "2026-08-25T18:18:09",
                    "shot_number": 1,
                    "club": "7-iron",
        ...
        got = ct.load_openflight(path)[0]
        assert got.attack_angle == pytest.approx(-4.0)
        assert got.club_path == pytest.approx(2.0)

    def test_openflight_prefers_the_fused_value_over_the_raw_candidate(self, tmp_path):
        """`experimental_attack_angle_deg` is a candidate that may be out of
        bounds; the fused field is the one the pipeline accepted. Comparing the
        candidate against truth would score a value the system rejected."""
        path = tmp_path / "of.jsonl"
        _write_openflight_jsonl(
            path,
            [
                {
                    ...
                    "shot_number": 1,
                    "club": "7-iron",
                    "experimental_attack_angle_deg": -33.4,
                    "experimental_fused_attack_angle_deg": -4.0,
                }
            ],
        )
        assert ct.load_openflight(path)[0].attack_angle == pytest.approx(-4.0)

    def test_rejected_club_values_are_not_silently_treated_as_measurements(self, tmp_path):
        """When the pipeline withholds a value the field must be absent, not
        zero. Radar club path is currently rejected on every shot."""
        path = tmp_path / "of.jsonl"
        _write_openflight_jsonl(
            path,
            [
        ...
        got = ct.load_openflight(path)[0]
        assert got.attack_angle is None
        assert got.club_path is None

    def test_comparison_csv_carries_club_columns_with_deltas(self, tmp_path):
        of_path, tm_path, out = tmp_path / "of.jsonl", tmp_path / "tm.csv", tmp_path / "c.csv"
        _write_openflight_jsonl(
            of_path,
            [
                {
        ...
        _write_trackman_csv(
            tm_path,
            ["Shot", "Club", "Ball Speed", "Attack Angle"],
            [{"Shot": "1", "Club": "7 Iron", "Ball Speed": "115", "Attack Angle": "-4.5"}],
        )
        pairs = ct.pair_shots(ct.load_openflight(of_path), ct.load_trackman(tm_path))
        ct.write_comparison_csv(pairs, out)
        with open(out, newline="", encoding="utf-8") as fh:
            row = next(iter(csv.DictReader(fh)))
        assert "attack_angle_of" in row and "attack_angle_tm" in row
        assert "face_angle_delta" in row, "every declared metric needs a delta column"
        assert float(row["attack_angle_delta"]) == pytest.approx(0.5)

    def test_club_metrics_appear_in_the_summary(self, capsys, tmp_path):
        of_path, tm_path = tmp_path / "of.jsonl", tmp_path / "tm.csv"
        _write_openflight_jsonl(
            of_path,
            [
                {
        ...
        _write_trackman_csv(
            tm_path,
            ["Shot", "Club", "Attack Angle"],
            [{"Shot": "1", "Club": "7 Iron", "Attack Angle": "-4.5"}],
        )
        pairs = ct.pair_shots(ct.load_openflight(of_path), ct.load_trackman(tm_path))
        ct.print_club_delivery_summary(pairs)
        out = capsys.readouterr().out
        assert "attack angle" in out.lower()
        assert "face angle" in out.lower(), "unmeasured metrics must be listed, not hidden"

    def test_summary_marks_metrics_openflight_cannot_yet_produce(self, capsys, tmp_path):
        """Face angle and impact location have no estimator yet. The summary
        must say so rather than reporting an empty comparison as agreement."""
        of_path, tm_path = tmp_path / "of.jsonl", tmp_path / "tm.csv"
        _write_openflight_jsonl(
            of_path,
            [
        ...
        _write_trackman_csv(
            tm_path,
            ["Shot", "Club", "Face Angle"],
            [{"Shot": "1", "Club": "7 Iron", "Face Angle": "-1.2"}],
        )
        pairs = ct.pair_shots(ct.load_openflight(of_path), ct.load_trackman(tm_path))
        ct.print_club_delivery_summary(pairs)
        out = capsys.readouterr().out
        assert "not produced" in out.lower() or "no openflight value" in out.lower()
