...
class TestBadValuesDegrade:
    ...
    def test_a_nan_humidity_degrades_rather_than_serving_a_nan_density(self):
        """NaN is the one bad value that survives clamping untouched, so it
        would otherwise poison the density silently rather than loudly."""
...
# ---------------------------------------------------------------------------
# Sensor tier. The classes below drove the sensor-only provider and now pin
# that the unified provider treats a fitted sensor as the top source.
# ---------------------------------------------------------------------------
