...
class OPS243Radar:
    ...
    def _drain_rearm_serial(self, quiet_period: float = 0.2):
        ...
        Internal triggering returns a complete I/Q dump without an ``S!``
        command. A short UART tail can still be present when the Q array is
        complete, so GC waits for a bounded quiet gap.
        """
        if not self.serial or not self.serial.is_open:
            raise ConnectionError("Not connected to radar")
    ...
    def configure_for_internal_speed_trigger(
        ...
        """Configure the OPS243's native speed-triggered rolling buffer.

        The board owns the trigger in this mode. GC enters rolling-buffer mode
        and resets detector settings, so every detector setting is restored
        while a temporary high outbound threshold prevents startup captures.
        """
        if not self.serial or not self.serial.is_open:
            raise ConnectionError("Not connected to radar")
        ...
        firmware_version = self.validate_internal_trigger_firmware()
        self._internal_trigger_firmware_version = firmware_version
        ...
        # GC resets the detector settings. Guard both sides of that transition
        # so retained settings cannot trigger a dump during restoration.
    ...
    def _restore_internal_speed_trigger_settings(self):
        """Restore all detector settings cleared by the last GC command."""
        ...
        self.set_sample_rate(sample_rate_ksps * 1000)
        self.set_units(SpeedUnit.MPH)
        # Once MPH is explicit, raise the temporary guard near the 30 ksps
        # ceiling while the remaining writes run.
        ...
        # Restore the requested trigger only after every other setting. ST and
        # SM require explicit carriage returns on this firmware path.
    ...
    def rearm_internal_speed_trigger(self, sample_rate_ksps: int = 30) -> bool:
        ...
        A write timeout is recoverable: the received capture remains valid and
        the next wait discards stale output until a fresh capture marker.
        """
        if not self.serial or not self.serial.is_open:
            raise ConnectionError("Not connected to radar")
        if sample_rate_ksps != 30:
