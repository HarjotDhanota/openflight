...
def detect_impact_reference_ball(
    ...
    *,
    ...
    pre_start = max(0, trigger_frame_index - 20)
    pre_stop = trigger_frame_index + 1
    if pre_stop - pre_start < 3:
        raise ValueError("impact-aware ball detection lacks pre-trigger frames")
    before = np.median(frames[pre_start:pre_stop], axis=0).astype(np.uint8)
    ...
    bright_count, _labels, bright_stats, bright_centroids = cv2.connectedComponentsWithStats(
        bright, 8
    )
    refinements = []
    ...
    *,
    ...
    for index in candidates:
        ...
        if len(after) == 2 and not after.any():
            return int(index)
    return None if trigger_frame_index is not None else int(indexes[-1])
