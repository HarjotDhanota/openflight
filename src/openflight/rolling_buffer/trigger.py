...
class HardwareTriggeredCapture(TriggerStrategy):
    """Capture using the OPS243-A native speed trigger."""
    ...
    def _decorate_latest_diagnostic(self, radar: "OPS243Radar", *, rearmed: bool) -> None:
        if not self._diagnostics:
            return
        self._diagnostics[-1].update(
            {
                "trigger_source": "ops_internal_speed",
                "firmware_version": getattr(radar, "_internal_trigger_firmware_version", None),
                "speed_threshold_mph": self.trigger_threshold_mph,
                "magnitude_threshold": self.trigger_magnitude,
                "pre_trigger_segments": self.pre_trigger_segments,
                "sample_rate_ksps": self.sample_rate_ksps,
                "rearmed": rearmed,
                "iwr_camera_fanout": "unavailable_no_qualified_trigger_edge",
            }
        )
    ...
    def wait_for_trigger(
        ...
        """Wait for one board-triggered dump, validate it, and re-arm."""
        logger.info(
...
def create_trigger(trigger_type: str = "sound", **kwargs) -> TriggerStrategy:
    ...
    Trigger types:
        ...
        - "hardware": OPS243 native speed trigger with rolling-buffer capture.
    """
    triggers = {
