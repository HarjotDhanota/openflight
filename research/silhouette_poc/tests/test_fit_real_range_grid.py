...
`fit_real.py` inherited a 1425 mm camera-to-ball range from a 13.97 px ball
...
from silhouette_poc.replay import fit_real
...
def test_measured_camera_preset_uses_the_tape_range():
    camera = fit_real.measured_camera()
    implied = fit_real.FOCAL_PX / camera.plate_scale_px_per_mm
...
def test_range_grids_bracket_the_true_range():
    ...
    for func in (fit_real.fit_frame, fit_real.fit_frame_6dof, fit_real.fit_sequence):
