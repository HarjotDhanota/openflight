...
def test_fit_sequence_can_keep_a_singleton_range_hard_pinned(monkeypatch):
    monkeypatch.setattr(fit_real, "CAMERA_CENTER_WORLD", np.zeros(3))
    monkeypatch.setattr(fit_real, "_ray_world", lambda _uv, _camera: np.asarray([1.0, 0.0, 0.0]))
    monkeypatch.setattr(
        fit_real,
        "render_mask_6dof",
