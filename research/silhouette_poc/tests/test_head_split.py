...
from silhouette_poc.replay.head_split import (
    HEAD_CORE_PX,
    SHAFT_MAX_PX,
    SHAFT_REACH_PX,
    split_head,
...
def test_merged_component_splits_at_the_neck():
    ...
    # every returned pixel comes from the observed component: never padded
    ...
    # the recovered head is the head, not the head plus a length of shaft
...
def test_merged_centroid_is_wrong_which_is_why_this_exists():
    ...
    # A short stub of shaft stays attached: the cut lands on the distance-
    # transform ridge, and in this fixture the shaft meets the ellipse
    # tangentially so there is no sharp constriction to land on. A real hosel
    # is a genuine neck and the residual is smaller. What matters is that the
    # centroid is the head's rather than the merged blob's.
